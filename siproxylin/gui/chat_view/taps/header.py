...
class ChatHeaderWidget(QFrame):
    ...
    def _request_room_membership(self, room_jid: str):
        ...
        # Create custom dialog for nickname and reason (non-blocking)
        dialog = QDialog(self)
        ...
        # Non-blocking: connect accepted signal to handler
        def on_accepted():
            ...
            if not nickname:
                ...
                msg.setText("Nickname cannot be empty")
                msg.show()
                return
    ...
    def _on_video_call_clicked(self):
        ...
        # Check if call functionality is available
        if not account.call_bridge or not account.jingle_adapter:
            logger.warning(f"Video call functionality not available for account {self.current_account_id}")
            QMessageBox.warning(
                self,
                "Calling Not Available",
        ...
        # Emit signal to parent
        logger.info(f"Emitting video call_requested signal for {self.current_jid}")
