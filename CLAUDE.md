DO NOT TOUCH GIT - do NOT use git writing operations like checkout, commit, merge, rebase, pull, fetch, push
DO NOT TOUCH BUILDS - do NOT run commands like make, cmake, gcc, g++, c++, ld - or similar
DO NOT TOUCH docs/ADR.md and CLAUDE.md files - unless expclicitely agreed with user to do modifications

FEEL FREE to do READ operations within the project scope: ls, cat, grep, awk, tr, sed or similar that needed to analyze logs or code itself
FEEL FREE to do web searches when necesscary


